          ...
          if (results) {
            ...
            async.eachSeries(results, function(result, callback) {
              ...
              assetmanager.retrieveAsset({ _id: result._assetId }, function (error, assets) {
                ...
                if(asset.isDirectory === true && asset.title.lastIndexOf(".")){
                  // if it's a directory, remove the file extension (if any exists)
                  asset.title = asset.title.substring(0, asset.title.lastIndexOf("."));
                }
                ...
                if (!processedAssets[asset.filename]) {
                  ...
                  // Check to see if the new name has the file extension of the unique name. Adds it if
                  // it's not there.
                  if (!asset.isDirectory && asset.title.substring(asset.title.lastIndexOf(".")) != asset.filename.substring(asset.filename.lastIndexOf("."))) {
                    asset.title = asset.title + asset.filename.substring(asset.filename.lastIndexOf("."));
                  }
                  ...
                  if(asset.isDirectory === false){
                    outputFilename = path.join(destinationFolder, asset.title);
                  } else {
                    var assetFolderName = asset.filename.substring(0, asset.filename.lastIndexOf("."));
                    outputFilename = path.join(destinationFolder, assetFolderName);
                  }
                  ...
                  jsonObject['component'] = JSON.parse(JSON.stringify(jsonObject['component']).replace(replaceRegex, "course/" + jsonObject['config']._defaultLanguage + "/assets/" + encodeURIComponent(asset.title)));
                  jsonObject['course'] = JSON.parse(JSON.stringify(jsonObject['course']).replace(replaceRegex, "course/" + jsonObject['config']._defaultLanguage + "/assets/" + encodeURIComponent(asset.title)));
                  ...
                  for (var i = 0; i < jsonObject['contentobject'].length; i++) {
                    if (jsonObject['contentobject'][i].hasOwnProperty('_graphic') && jsonObject['contentobject'][i]._graphic.hasOwnProperty('src')
                      ...
                      jsonObject['contentobject'][i]._graphic.src = "course/" + jsonObject['config']._defaultLanguage + "/assets/" + encodeURIComponent(asset.title);
                    }
                    if (jsonObject['contentobject'][i].hasOwnProperty('_media')){
                      if(jsonObject['contentobject'][i]._media.hasOwnProperty('mp4') && jsonObject['contentobject'][i]._media.mp4 == "course/assets/" + asset.filename) {
                        jsonObject['contentobject'][i]._media.mp4 = "course/" + jsonObject['config']._defaultLanguage + "/assets/" + encodeURIComponent(asset.title);
                      }
                      if(jsonObject['contentobject'][i]._media.hasOwnProperty('poster') && jsonObject['contentobject'][i]._media.poster == "course/assets/" + asset.filename) {
                        jsonObject['contentobject'][i]._media.poster = "course/" + jsonObject['config']._defaultLanguage + "/assets/" + encodeURIComponent(asset.title);
                      }
                    }
                  }
                  ...
                  filestorage.getStorage(asset.repository, function (err, storage) {
                    ...
                    // if the asset is a directory, copy the entire folder out
                    if(asset.isDirectory === true){
                      return storage.copyFolder(asset.path, outputFilename, callback);
                    }
